...
var create = function (req, res, Model, data, meta) {
  return new Promise((resolve, reject) => {
    async.reduce(_.keys(Model.attributes), [], function (memo, attrName, nextAttrName) {
      var attrDef = Model.attributes[attrName];

      if (attrDef.model && _.isObject(data[attrName])) {
        create(req, res, req._sails.models[attrDef.model], data[attrName], meta).then((newInstance) => {
            data[attrName] = newInstance[Model.primaryKey];
            nextAttrName(undefined, memo);
          })
          .catch((err) => {
            reject(err);
          });
      } else if (attrDef.collection && _.isArray(data[attrName]) && data[attrName].length > 0) {
        var promisses = data[attrName].map((items) => {
          return create(req, res, req._sails.models[attrDef.collection], items, meta);
        });

        Promise.all(promisses).then((newInstances) => {
          data[attrName] = newInstances.map(function (el) {
            return el[Model.primaryKey];
          });
      ...
      if (err) reject(err);
        ...
        if (_.isArray(data) && data.length > 0) {
          Model.createEach(data).meta(meta).exec(function created(err, newInstance) {
            if (err) reject(err);
            else resolve(newInstance);
          });
        } else {
          Model.create(data).meta(meta).exec(function created(err, newInstances) {
            if (err) reject(err);
            else resolve(newInstances);
          });
        }
      }
    });
...
module.exports = function createRecord(req, res) {
  ...
  async.reduce(_.keys(Model.attributes), [], function (memo, attrName, nextAttrName) {
    ...
    if (
      ...
      req._sails.models[attrDef.collection].attributes[attrDef.via] &&
    ...
    create(req, res, Model, data, queryOptions.meta).then((newInstance) => {
