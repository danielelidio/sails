...
module.exports = function createRecord(req, res) {
  ...
  // TODO: WRITE TESTS; CHECK FOR ERRORS THAT COULD OCCOUR.
  async.reduce(_.keys(Model.attributes), [], function (memo, attrName, nextAttrName) {
    var attrDef = Model.attributes[attrName];
    if (attrDef.collection && _.isArray(data[attrName]) && data[attrName].length > 0) {
      req._sails.models[attrDef.collection].createEach(data[attrName]).meta(queryOptions.meta).exec(function created(err, newInstance) {
        if (err) throw err;
        data[attrName] = newInstance.map(function (el) {
          return el.id;
        });
